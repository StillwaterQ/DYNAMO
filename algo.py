...
class dynamo():
    def __init__(self, input_cir_name):
        self.input_cir_name = input_cir_name
        self.space_cirs = []
        self.row_per_site = 3
        self.all_aod = False
        ...
        self.result_json = {}
        self.result_json['name'] = input_cir_name
    ...
    def setArchitecture(self, architecture):
        self.n_x, self.n_y, self.n_c, self.n_r = architecture[0], architecture[1], architecture[2], architecture[3]

    def setProgram(self, gate_list, nqubit: int = None):
        self.n_g = len(gate_list)
        self.g_i = [i for i in range(self.n_g)]
        tmp = [(min(pair), max(pair)) for pair in gate_list]
        self.g_q = tuple(tmp)
        ...
        if not nqubit:
            ...
            self.n_q = nqubit
        self.dependencies = self.dependencyExtract(self.g_q, self.n_q)
        self.n_t = self.pushLeftDepth(self.g_q, self.n_q)
        self.collisions = self.collisionExtract(self.g_q)
        self.updateGateIndexMatrix()

    def dependencyExtract(self, list_gate_qubits, count_program_qubit: int):
        list_dependency = []
    ...
    def pushLeftDepth(self, list_gate_qubits, count_program_qubit: int) -> int:
    ...
    def collisionExtract(self, list_gate_qubits):
        list_collision = list()
    ...
    def maxDegree(self, list_gate_qubits, count_program_qubit: int) -> int:
    ...
    def setCommutation(self):
        ...
        self.n_t = self.maxDegree(self.g_q, self.n_q)
        self.dependencies = []
    ...
    def solve(self):
        ...
        with open(f'{self.save_dir}/' + f"{self.result_json['name']}.json", 'w') as f:
    ...
    def solve_greedy(self):
        total_g_q = len(self.g_q)
        t_curr = 1
        while len(self.g_q) > settings.optimal_gates_boundary:
            step = 1
            ...
            curr_layer_idx = len(self.result_json['layers'])
            num_stages = step + 1
            self.irregular_constraints(curr_layer_idx = curr_layer_idx,
                                       num_stages = num_stages,
                                       a=a, c=c, r=r, x=x, y=y, t=t)
    ...
    def init_space_gateforbidden(self):
        # Initializes the data structure to store gate-induced forbidden zones
        # derived from the loaded spatial circuits.
        pass

    def add_space_circuit(self, space_json_file):
        # Loads and processes a pre-compiled circuit from a JSON file.
        # This circuit's layout, qubit movements, and gate locations will be
        # used as a dynamic spatial constraint (an 'irregular' hardware shape)
    ...
    def irregular_constraints(self, curr_layer_idx, num_stages, a, c, r, x, y, t):
        # This is the main function for applying all 'irregular' spatial constraints
        # to the Z3 solver. It iterates over the loaded spatial circuits
        # and adds constraints related to forced qubit movements (AOD moves)
        # and gate-forbidden zones for the current compilation batch.
        pass
    ...
    def constraint_irregular_aodmove(self, stage0_idx, stage1_idx, start_site, end_site, x, y):
        # Adds specific Z3 constraints to the solver to enforce qubit movement
        # trajectories. This ensures that any qubit occupying a site within a
        # predefined movement path (from start_site to end_site) must follow
        # that path, maintaining the relative spatial ordering.
        pass

    def constraint_gateforbidden(self, x0, y0, stage_idx, x, y, t):
        # Adds specific Z3 constraints to the solver to enforce gate-forbidden zones.
        # This prevents the scheduler from placing any new gate (via the 't' variable)
        # at a specific site (x0, y0) during a specific stage (stage_idx) if that
        # site is already occupied by a gate in one of the loaded spatial circuits.
        pass
    ...
    def init_space_circuit(self):
            # Initializes the data structure to store spatial circuit layer definitions.
            pass
    ...
    def init_space_qmovement(self):
        # Initializes the data structure to store qubit movement trajectories
        # derived from the loaded spatial circuits.
        pass
